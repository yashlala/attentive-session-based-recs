...
class GRU4REC(nn.Module):
    """Implements vanilla GRU4REC."""
    def __init__(self, input_size, hidden_size, output_size, num_layers=1,
            final_activation_function='tanh', dropout_hidden=.5,
            dropout_input=0, batch_size=50, embedding_dim=-1, use_cuda=False):
    ...
    def create_final_activation(self, final_act):
        """Sets the final activation function for the network output."""
        if final_act == 'tanh':
    ...
    def forward(self, input, hidden):
        """Compute a single forward pass of GRU4REC.

        Args:
            input (B,): a batch of item indices from a session-parallel mini-batch.
            target (B,): torch.LongTensor of next item indices from a session-parallel mini-batch.
        Returns:
            logit (B,C): Variable that stores the logits for the next items in
                         the session-parallel mini-batch
    ...
    def init_emb(self):
        """Initialize the one_hot embedding buffer.

        This will be used for producing the one-hot embeddings efficiently.
        """
    ...
    def onehot_encode(self, input):
        """Return a one-hot vector corresponding to the input.
