...
class GRUDatasetGenres(Dataset):
    def __init__(self, u2seq, genre_df, null_value, mode='train',max_length=200,pad_token=0):
        """
        Parameters
        ----------
        u2seq : dictionary
            Dictionary containing all user's session history. Key is user id, value is list of user clicks [102, 1203 ...] .
        genre_df : DataFrame
            Dataframe with the embeddings of genres and item ids
        null_value: int
            embedding of the null value for genre
        mode : string, optional
            Whether the dataloader is used for training "train" or for validation/testing "eval". The default is 'train'.
            If eval, uses only last item in sequence for label
        max_length : integer, optional
        ...
        self.pad_token = pad_token
        self.mode=mode
        self.genres = genre_df
        self.null_val = null_value
        
        # adding pad genres
        len_genres = len(genre_df['genre'][0])
        self.genres.loc[len(self.genres.index)] = [pad_token, [null_value] * len_genres] 
    ...
    def __getitem__(self, index):
        ...
        genres = np.vstack(self.genres.loc[tokens]['genre'].to_numpy())
        
        
        return (torch.LongTensor(tokens), 
                torch.LongTensor(genres), 
                torch.LongTensor(labels),
                torch.LongTensor([x_len]),
